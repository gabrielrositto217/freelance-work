SELECT 
	...
	/* 
	Gabriel@Barney: Careful with this as it will be a number ranging from 0 to 1, not 0 to 100. Writing a fix and leaving the original one just in case.
	Anyway this could potentially have format issues, probably the best is to just send this as a number (i.e. 0.12) and let the devs handle the formatting on the front-end
	*/
    -- CONCAT(round(conv.subscriptions / pt.visits,2),'%') as 'Conversion Rate',
	CONCAT(ROUND(conv.subscriptions * 100.00 / pt.visits, 2), '%') as 'Conversion Rate',
	
	-- Gabriel@Barney: commented these out as source table changed
--    postbacks.postback_sent as 'Postback Sent',
--    postbacks.postback_declined as 'Postback Declined',
	pt2.postback_sent as 'Postback Sent',
	pt2.postback_declined as 'Postback Declined',
...
/*
		Gabriel@Barney: Reworked this section to use only one table called 'pt2'
		(SELECT 
...
*/
		...
		(SELECT 
        SUM(CASE WHEN ph.skimmed = 0 THEN c.cost ELSE 0 END) AS payout,
		COUNT(CASE WHEN ph.skimmed = 0 THEN ph.id END) as postback_sent,
		COUNT(CASE WHEN ph.skimmed = 1 THEN ph.id END) as postback_declined,
		s.campaign_id
    FROM
